...
func NewTelemetryBuilder(settings component.TelemetrySettings, options ...TelemetryBuilderOption) (*TelemetryBuilder, error) {
	...
	builder.ExporterDroppedLogRecords, err = builder.meter.Int64Counter(
		...
		metric.WithDescription("Number of log records permanently dropped (queue full, retries exhausted, permanent errors, etc.). [alpha]"),
		metric.WithUnit("{records}"),
	)
	errs = errors.Join(errs, err)
	builder.ExporterDroppedMetricPoints, err = builder.meter.Int64Counter(
		"otelcol_exporter_dropped_metric_points",
		metric.WithDescription("Number of metric points permanently dropped (queue full, retries exhausted, permanent errors, etc.). [alpha]"),
		metric.WithUnit("{datapoints}"),
	)
	errs = errors.Join(errs, err)
	builder.ExporterDroppedSpans, err = builder.meter.Int64Counter(
		"otelcol_exporter_dropped_spans",
		metric.WithDescription("Number of spans permanently dropped (queue full, retries exhausted, permanent errors, etc.). [alpha]"),
